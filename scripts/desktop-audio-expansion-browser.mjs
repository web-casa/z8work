...
state.tasks=[state.tasks[0]];state.tasks[0].name='input.${format}';state.tasks[0].format='${format}';state.tasks[0].formats=['wav','mp3','flac','opus','m4a','ogg','aiff'];state.tasks[0].phase='ready';state.tasks[0].result=null;state.import_report=null;
window.calls=[];
mockIPC(async(command,args)=>{
 ...
 if(command==='desktop_info') return {preparing:false,startup:[{id:'ffmpeg',phase:'ready',failure:null},{id:'ffprobe',phase:'ready',failure:null}],pending_imports:0,import_failure:null,workspace_error:null,temporary_cleanup:null,architecture:'test',engines:[],routes:[{input:'${format}',outputs:['wav','mp3','flac','opus','m4a','ogg','aiff']}],error:null,queue_error:null};
...
`;
		...
		assert.equal(await page.getByText(/仅保留第一条音轨。OGG/).count(), 2);
		assert.equal(
		...
		const taskSelect = page
			.locator("select")
			.filter({ has: page.locator('option[value="aiff"]') })
			.last();
		...
		await page.screenshot({
			path: `.desktop-local/format-phase3b-browser/${format}.png`,
			fullPage: true,
		});
		await page.close();
		console.log(format + ": audio controls passed");
	}
} finally {
	await browser.close();
	await server.close();
