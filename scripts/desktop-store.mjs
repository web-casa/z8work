...
if (values.output) {
	...
	await writeFile(
		...
		`# Z8.Work desktop store dossier\n\nStatus: ${report.status}. These are local drafts, not an API submission payload. Public page URLs are planned until deployment is verified. There are no store screenshots attached: the Phase 3 Linux ARM64 screenshot is reference evidence only.\n\n${channels.map((c) => `## ${c.channel}\n\n${c.blockers.map((b) => `- ${b}`).join("\n")}`).join("\n\n")}\n\nReview the current Partner Center/Snap forms manually. No login, upload, certification or publication has been performed.\n`,
	);
}
